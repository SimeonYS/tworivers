...
class TtworiversPipeline:

    # Database setup
    conn = sqlite3.connect('tworivers.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `tworivers`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM tworivers WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO tworivers (date, title, link, content)"
